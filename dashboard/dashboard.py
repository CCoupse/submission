...
def create_daily_usage_df(df):
    daily_usage_df = df.groupby("dteday").agg({"cnt": "sum"}).reset_index() # Perubahan: Sum 'cnt' untuk total harian
    return daily_usage_df

def create_seasonal_usage_df(df):
    seasonal_usage_df = df.groupby("season").agg({"cnt": "sum"}).reset_index() # Perubahan: Sum 'cnt' untuk total musiman
...
def create_monthly_usage_df(df):
    monthly_usage_df = df.groupby("mnth").agg({"cnt": "sum"}).reset_index() # Perubahan: Sum 'cnt' untuk total bulanan
...
def create_yearly_usage_df(df):
    yearly_usage_df = df.groupby("yr").agg({"cnt": "sum"}).reset_index() # Perubahan: Sum 'cnt' untuk total tahunan
...
with st.expander("Perbandingan Kondisi Cuaca Rata-rata Berdasarkan Klaster Penyewaan Sepeda"):
    ...
    fig_cluster_weather_bar = px.bar(cluster_weather_melted, x='rental_cluster', y='average_value', color='weather_feature',
                                     ...
                                     color_discrete_sequence=px.colors.qualitative.Vivid) # Palet warna Vivid untuk kontras
...
with st.expander("Distribusi Kondisi Cuaca (Weathersit) dalam Klaster Penyewaan Sepeda"):
    ...
    fig_weathersit_cluster_bar = px.bar(weathersit_cluster_counts_melted, x='rental_cluster', y='proportion', color='weathersit_label',
                                        ...
                                        color_discrete_sequence=px.colors.qualitative.Pastel) # Palet Pastel untuk weathersit
